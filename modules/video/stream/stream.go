...
func (s *stream) HandleVideoCreated(ctx context.Context, req *pb.HandleVideoCreatedRequest) (*emptypb.Empty, error) {
	...
	if req.GetScale() != 0 {
		...
		// [Kafka TODO]
		// [Describe] Transcode video if get message with scale != 0, you can handle error occurrance like above primitive.ObjectIDFromHex(req.GetId()).
	...
	// [Kafka TODO]
	// [Describe] Fanout create events to each variant [1080, 720, 480, 320], you can handle error occurrance like above primitive.ObjectIDFromHex(req.GetId()).
